    ...
    with _lock_escrita_log:
        try:
            with open(caminho_log, "a", encoding="utf-8") as f:
                f.write(
                    f"| {timestamp} | `{componente}` | **{evento}** | {msg_limpa} |\n"
                )
...
def log_evento(nivel: str, componente: str, evento: str, mensagem: str) -> None:
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(
        f"[{timestamp}] {nivel}[{componente} -> {evento}]{Cores.RESET} {mensagem}{Cores.RESET}"
    )
...
def exibir_health_check() -> None:
    ...
    for nome, online in saude.items():
        porta = NOS_GRPC[nome]
        status = (
            f"{Cores.SUCCESS}ONLINE{Cores.RESET} "
            if online
            else f"{Cores.FAIL}OFFLINE{Cores.RESET}"
        )
...
if __name__ == "__main__":
    ...
    if arg == "--status":
        ...
        print(
            f"Branch: {Cores.CYAN}{BRANCH_ATUAL}{Cores.RESET} | Operador: {Cores.CYAN}{USUARIO_ATUAL}{Cores.RESET}"
        )
        exibir_health_check()
