...
class ControlClient:
    ...
    def heartbeat(self, node_id, free_space_bytes, active_uploads, active_downloads, chunk_ids):
        return self.stub.Heartbeat(dfs_pb2.HeartbeatRequest(
            node_id=node_id, free_space_bytes=free_space_bytes,
            active_uploads=active_uploads, active_downloads=active_downloads,
            chunk_ids=list(chunk_ids)), timeout=self.timeout)

    def confirm_upload(self, upload_id, chunks, total_size_bytes):
        return self.stub.ConfirmUpload(dfs_pb2.ConfirmUploadRequest(
            upload_id=upload_id, chunks=list(chunks),
            total_size_bytes=total_size_bytes), timeout=self.timeout)

    # ----- usado pela CLI -----
    def request_upload(self, logical_path, total_size_bytes, client_request_id=""):
        return self.stub.RequestUpload(dfs_pb2.RequestUploadRequest(
            logical_path=logical_path, total_size_bytes=total_size_bytes,
            client_request_id=client_request_id), timeout=self.timeout)

    def request_download(self, logical_path, client_request_id=""):
        return self.stub.RequestDownload(dfs_pb2.RequestDownloadRequest(
            logical_path=logical_path, client_request_id=client_request_id),
            timeout=self.timeout)

    def delete_file(self, logical_path):
        return self.stub.DeleteFile(dfs_pb2.DeleteFileRequest(logical_path=logical_path),
                                    timeout=self.timeout)
